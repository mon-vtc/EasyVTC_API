...
describe('PricingService', () => {
  ...
  describe('getActiveGrid()', () => {
    it('retourne la grille active', async () => {
      mockChain(mockGridFrance);
      const grid = await service.getActiveGrid();
      expect(grid.is_active).toBe(true);
      expect(grid.base_price).toBe(2.50);
    });

    it('lève une erreur 404 si aucune grille active', async () => {
      mockChain(null, { message: 'No rows found' });
      await expect(service.getActiveGrid()).rejects.toMatchObject({ status: 404 });
    });
  });

  // ────────────────────────────────────────────────────────────────────────────
  // Calcul de prix — Formule
  // ────────────────────────────────────────────────────────────────────────────
  ...
  describe('calculatePrice() — mode formule', () => {
    ...
    it('calcule correctement un trajet standard', async () => {
  ...
  describe('calculatePrice() — mode forfait', () => {
    ...
    it('lève une erreur si le forfait est inactif', async () => {
      ...
      await expect(
        service.calculatePrice({ flat_rate_id: 'fr-1' }),
