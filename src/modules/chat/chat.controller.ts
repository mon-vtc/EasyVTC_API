...
export class ChatController {
  ...
  async listConversations(req: Request, res: Response): Promise<void> {
    const parsed = conversationListFiltersSchema.safeParse(req.query);
    if (!parsed.success) {
  ...
  async createSupportTicket(req: Request, res: Response): Promise<void> {
    ...
    try {
      const ticket = await chatService.createSupportTicket(
        req.user!.id,
        req.user!.role,
        parsed.data,
      );
      res.status(201).json({ ok: true, message: 'Ticket de support créé', data: ticket });
    } catch (err: unknown) {
      const e = err as { status?: number; message?: string };
      res.status(e.status ?? 500).json({ ok: false, message: e.message ?? 'Erreur serveur' });
    }
  ...
  // GET /support/tickets
  async listSupportTickets(req: Request, res: Response): Promise<void> {
    const parsed = supportListFiltersSchema.safeParse(req.query);
    if (!parsed.success) {
      res.status(400).json({ ok: false, message: 'Filtres invalides', errors: parsed.error.flatten().fieldErrors });
      return;
    }
    try {
      const result = await chatService.listSupportTickets(
        req.user!.id,
        req.user!.role,
        parsed.data,
      );
  ...
  async getSupportTicketDetail(req: Request, res: Response): Promise<void> {
    const parsed = supportTicketParamsSchema.safeParse(req.params);
    if (!parsed.success) {
      res.status(400).json({ ok: false, message: 'ID de ticket invalide' });
      return;
    }
    try {
      const ticket = await chatService.getSupportTicketDetail(
  ...
  async updateSupportTicketStatus(req: Request, res: Response): Promise<void> {
    const paramsParsed = supportTicketParamsSchema.safeParse(req.params);
    if (!paramsParsed.success) {
      res.status(400).json({ ok: false, message: 'ID de ticket invalide' });
      return;
    }
    const bodyParsed = updateSupportStatusSchema.safeParse(req.body);
    if (!bodyParsed.success) {
      res.status(400).json({ ok: false, message: 'Données invalides', errors: bodyParsed.error.flatten().fieldErrors });
      return;
    }
    try {
      const ticket = await chatService.updateSupportTicketStatus(
        paramsParsed.data.ticketId,
